...
const vec4 k_a = vec4(0.1, 0.2, 0.3, 0.4);   // More transparent ambient
const vec4 k_d = vec4(0.1, 0.2, 0.3, 0.3);   // Transparent diffuse
...
//    vec4 phongColor = (LightAmbient * k_a) + (LightDiffuse * k_d * L_dot_N) + (LightSpecular * k_s * pow(R_dot_V, n));
    ...
    vec4 blinnPhong = fresnel * k_s * specularIntensity + (LightAmbient * k_a) + (LightDiffuse * k_d * L_dot_N);

    // light scattering
    float firstTerm = 0.5 * max(0.05, waveHeight) * pow(max(0.0, dot(L, V)), 4) * pow(0.5 - 0.5 * L_dot_N, 3); //scatter due to waveheight
    float secondTerm = 1.8 * pow(max(0.0, dot(V, N)), 2); // how visible normal is to camera
    float thirdTerm = 2.0 * L_dot_N; // Lambert cosine
    vec3 ambient = 0.8 * k_a.xyz; // ambient light
